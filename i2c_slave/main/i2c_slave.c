...
void app_main(void)
{
    ...
    i2c_slave_config_t i2c_slv_config = {          
        // .addr_bit_len = I2C_ADDR_BIT_LEN_7,      // 如果您需要从机具有 10 位地址，则 addr_bit_len 设置为 true
        .clk_source = I2C_CLK_SRC_DEFAULT,       // 选择I2C总线的源时钟
        .i2c_port = PORT_NUMBER,                 // 设置控制器使用的 I2C 端口
        .send_buf_depth = 256,                   // 设置发送缓冲区长度
        .scl_io_num = SCL_IO_PIN,                // 设置串行时钟总线 (SCL) 的 GPIO 编号
        .sda_io_num = SDA_IO_PIN,                // 设置串行数据总线 (SDA) 的 GPIO 编号
        .slave_addr = 0x58,                      // 设置从机地址
    };
    ...
    uint8_t *data_wr = (uint8_t *) malloc(LENGTH);

    for (int i = 0; i < LENGTH; i++) {
        data_wr[i] = i;
    }

    ESP_ERROR_CHECK(i2c_slave_transmit(slave_handle, data_wr, LENGTH, 10000));
